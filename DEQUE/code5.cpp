...
bool equal(){
	if(q.size() != stl.size()) {std::cout<<q.size()<<" "<<stl.size()<<std::endl;return 0;}
	if(q.empty() != stl.empty()){std::cout<<" empty";return 0;}
	int cnt = 0;
...
void test5(){
	...
	if(!equal())
        {puts("Wrong Answer1");return;}
	it_q = q.erase(q.end() - 1);
	it_stl = stl.erase(stl.end() - 1);
	if(!equal())
	  {puts("Wrong Answer1.755");return;}
	if(it_q != q.end())
	   {puts("Wrong Answer2");return;}
	...
	if(it_q != q.end() - 1)
        {puts("Wrong Answer3");return;}
	if(!equal())
	  {puts("Wrong Answer4");return;}
	puts("Accept");
}
void test6(){
