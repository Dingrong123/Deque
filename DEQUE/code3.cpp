...
std::pair<bool, double> synthesisChecker() {
	...
	for (int i = 0, delta; i < N; i++) {
		...
		delta = itA - a.begin();
		int delta1=itB-b.begin();
		if (b.begin() + delta!=itB) {
		     std::cout<<"false8"<<" "<<itB-(b.begin() + delta)<<std::endl;
			return std::make_pair(false, 0);
		}
		if (*itA != *itB) {
	...
	if (!isEqual(ttA, ttB)) {
	     std::cout<<"false13"<<std::endl;
		return std::make_pair(false, 0);
	}
	if (!isEqual(tttA, tttB)) {
	     std::cout<<"false14"<<std::endl;
		return std::make_pair(false, 0);
	}
	...
	for (int i = 0, delta; i < N; i++) {
		...
		delta = itA - a.begin();
		//std::cout<<"whynot16"<<std::endl;
		if (b.begin() + delta != itB) {
		     std::cout<<"false16"<<std::endl;
			return std::make_pair(false, 0);
		}
		if (itA == a.end()) {
			itA--;
		...
		itB = b.erase(itB);
		delta = itA - a.begin();
		//std::cout<<"whynot17"<<std::endl;
		if (b.begin() + delta != itB) {
		     std::cout<<"false17"<<std::endl;
			return std::make_pair(false, 0);
		}
	}
	if (!isEqual(a, b)) {
         std::cout<<"false18"<<std::endl;
		return std::make_pair(false, 0);
	}
	if (!isEqual(tA, tB)) {
	     std::cout<<"false19"<<std::endl;
		return std::make_pair(false, 0);
	}
	if (!isEqual(ttA, ttB)) {
	     std::cout<<"false20"<<std::endl;
		return std::make_pair(false, 0);
	}
